...
export default function C6() {
  ...
  const sendMessageToBackend = async (payload: {
    ...
    try {
      ...
      const response = await fetch(
        "http://192.168.1.174:3000/9fc93aca-a44f-0f64-bfae-2ed40110f88b/message",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            userId: user.uid, // Include user-specific data if needed
      ...
      return data || "Sorry, I couldn't process that.";
    } catch (error) {
      console.error("Error sending message to backend:", error);
      return "Sorry, something went wrong."; // Fallback message
    }
  };
  ...
  const uploadImageToBackend = async (uri: string, text: string) => {
    try {
      const formData = new FormData();
      formData.append("userId", user?.uid || "test-user");
      formData.append("roomId", `default-room-${user?.uid}`);

      const fileName = uri.split("/").pop();
      const fileType = fileName?.split(".").pop();

      formData.append("file", { uri, name: fileName || `upload.${fileType}`, type: `image/${fileType}` });

      const response = await fetch("http://192.168.1.174:3000/9fc93aca-a44f-0f64-bfae-2ed40110f88b/message", {
        method: "POST",
        body: formData,
      ...
      const imageMessage: IMessage = {
        ...
        image: data[0].url, // Backend returns the image URL
        text: text,
      };
  ...
  const handleSendImageMessage = async () => {
    ...
    setText("");
    Keyboard.dismiss();
    const replyText = await sendMessageToBackend({
      text: tempText,
      fileId: tempImageId,
    });
    ...
    for (var reply of replyText) {
      ...
      if (count > 0) {
        ...
        //   roomId: user.uid,
        // });
        await uploadImageToBackend(image, replyMessage.text);
