...
const QuickLogBox = ({ date }: { date: Date }) => {
  ...
  return (
    <View>
      {/* Upload Box */}
      <View className="px-6 my-4 flex flex-row gap-4 justify-center">
        <TouchableOpacity
          onPress={pickImage}
          className="border-2 border-dashed border-newblue rounded-lg p-5 items-center justify-center"
        >
          <Text className="text-newblue">+ Quick Add Meal</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setShowChatBox(true)}
          className="border-2 border-dashed border-newblue rounded-lg p-5 items-center justify-center"
        >
          <Text className="text-newblue text-center">+ Quick Manual Log</Text>
        </TouchableOpacity>
      </View>

      {/* Chat Box */}
      {showChatBox && (
        <View className="bg-white mx-6 mt-4 p-4 rounded-xl shadow">
          {image && (
            <Image
              source={{ uri: image }}
              style={{
